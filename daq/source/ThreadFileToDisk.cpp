...
//		snprintf(datafilename, sizeof(datafilename), "%s/testdata%d.bin", m_strFilePathPing.data(), file_cnt);
//
//		HANDLE f = CreateFileA(datafilename, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_FLAG_NO_BUFFERING, NULL);
//
//		if (f == INVALID_HANDLE_VALUE) 
...
UINT ThreadFileToDisk::SingleFilePong(LPVOID lParam)
{
	...
	{
		if (ThreadFileToDisk::Ins().m_bInterrupt && ThreadFileToDisk::Ins().GetAvailSizePong() == 0)
			break;
...
//
//	char datafilename[512] = { 0 };
//	snprintf(datafilename, sizeof(datafilename), "%sxdma0.bin", m_strFilePathPing.data());
//
//	HANDLE f = CreateFileA(datafilename, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_FLAG_NO_BUFFERING, NULL);
//
//	if (f == INVALID_HANDLE_VALUE) {
...
//
//            char datafilename[512] = { 0 };
//			snprintf(datafilename, sizeof(datafilename), "%sxdma%d.bin", m_strFilePathPing.data(), databuf.m_iBufferIndex);
//
